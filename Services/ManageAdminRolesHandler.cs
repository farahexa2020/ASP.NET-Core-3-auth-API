    ...
    protected override Task HandleRequirementAsync(AuthorizationHandlerContext context, ManageAdminRolesRequiremnet requirement)
    {
      try
      {
        ...
        using (var scope = this.serviceProvider.CreateScope())
        {
          ...
          isEditedUserAdmin = userManager.IsInRoleAsync(user, RolesEnum.Admin.ToString()).Result;

          var adminRoleId = roleManager.GetRoleIdAsync(roleManager.FindByNameAsync(RolesEnum.Admin.ToString()).Result).Result;
          isRoleAdmin = roleId.ToString().ToLower() == adminRoleId.ToString().ToLower();
        }

        if (
            (context.User.HasClaim(ClaimTypes.Role, ClaimValuesEnum.SuperAdmin.ToString())) ||
            (context.User.IsInRole(RolesEnum.Admin.ToString()) && !isEditedUserAdmin && !isRoleAdmin)
          )
        {
